...
install_nginx() {
  ...
  if [ -z "$DOMAIN" ]; then
    ...
    read -rp "  Domain name (e.g. garderobe.example.com): " DOMAIN
    [ -z "$DOMAIN" ] && err "Domain is required for nginx setup"
  fi
...
# Generated by install.sh – safe to commit (no secrets)
...
server {
    ...
    # SSL certs managed by certbot (placeholders until certbot runs)
...
EOF
  ...
  # Update ORIGIN in .env to use the real domain
